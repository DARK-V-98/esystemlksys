...
import { ArrowLeft, Unlock, Download, Cpu, CheckCircle, Trash2 } from 'lucide-react';
...
export default function UnlockPdfPage() {
  ...
  const handleUnlock = async () => {
    ...
    if (!password) {
      toast.error('Please enter the password for the PDF.');
      return;
    }

    setIsProcessing(true);
    toast.info('Unlocking PDF...');
    ...
    try {
      ...
      console.error(error);
      toast.error('Failed to unlock PDF. Please double-check the password.');
    } finally {
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-primary-foreground">
              Unlock <span className="text-primary neon-text">PDF</span>
            </h1>
        ...
        <div className="bg-secondary/50 border rounded-lg p-6 space-y-6">
            {!file ? (
                ...
                <div className="space-y-4">
                    ...
                    <Button onClick={handleUnlock} disabled={isProcessing || !password} className="w-full" variant="gradient">
                        {isProcessing ? <Cpu className="mr-2 h-5 w-5 animate-spin" /> : <Unlock className="mr-2 h-5 w-5" />}
                        {isProcessing ? 'Unlocking...' : 'Unlock PDF & Download'}
                    </Button>
                </div>
            )}
        </div>
