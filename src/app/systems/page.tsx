        ...
        <div className="bg-card rounded-lg border shadow-sm p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
              {businessSystems.map((system, index) => (
                <Link 
                  key={system.id}
                  href={`/coming-soon?title=${encodeURIComponent(system.name)}`}
                  className="system-card group rounded-2xl p-8 text-center shadow-card h-full flex flex-col items-center justify-center aspect-video animate-slide-up"
